    ...
    <el-row style="margin-left: 40px" divider>
      ...
      <el-col :span="4" class="user-info">
        ...
        <el-card class="prompts">
          ...
          <el-col style="margin-top: 25px">
            ...
            <p>
              <span style="color: salmon">转会窗口</span
              ><span>(冬窗/夏窗期)</span>
            </p>
      ...
      <el-col :span="6" class="user-info" :offset="1">
        ...
        <el-card v-if="inputcard_visible" class="prompts" style="height: 200px">
          <h3 v-if="this.currentKeywordIndex === 0" class="title">
            补充转会费(万元)
          </h3>
          <h3 v-else-if="this.currentKeywordIndex === 1" class="title">
            补充工资(万元)
          </h3>
          <h3 v-else-if="this.currentKeywordIndex === 3" class="title">
            补充合同时长(年)
          </h3>
        ...
        <el-card v-if="form_visible" class="prompts">
          <el-form :model="transferDetails" label-width="60px">
            ...
            <el-form-item label="窗口">
              <el-select
                v-model="transferDetails.transferWindow"
                placeholder="请选择"
              >
                <el-option
                  v-for="item in options"
...
export default {
  ...
  data() {
    return {
      ...
      messages: [
        {
          user: "manager",
          text: `我们球队有一个球员${this.value.playerName}可以考虑转会。`,
        },
  ...
  methods: {
    ...
    checkKeywordSequence(inputText) {
      ...
      if (inputText.includes(currentKeyword)) {
        ...
        if (this.currentKeywordIndex < this.requiredKeywords.length) {
          ...
          this.sendPrompt(
            this.requiredKeywords[this.currentKeywordIndex],
            true
          );
        }
      } else {
    ...
    handleKeywordInput(keyword, inputText) {
      ...
      if (keyword === "转会费") {
        ...
        if (this.inputdata !== null)
          this.transferDetails.salary = this.inputdata;
      } else if (keyword === "转会窗口") {
        if (this.inputdata.includes("冬窗"))
          this.transferDetails.transferWindow = "冬窗";
        else if (this.inputdata.includes("夏窗"))
          this.transferDetails.transferWindow = "夏窗";
      } else if (keyword === "合同时长") {
        if (this.inputdata !== null)
          this.transferDetails.contractDuration = this.inputdata;
      }
    ...
    checkAllDetailsConfirmed() {
      if (
        this.transferDetails.fee !== null &&
        this.transferDetails.salary !== null &&
        this.transferDetails.transferWindow &&
        this.transferDetails.contractDuration !== null
      ) {
        this.checkTransfer();
        // 检测是否同意转会
    ...
    checkTransfer() {
      ...
      // 确定合同到期时间，使用合同期限增加年份
      const endYear =
        today.getFullYear() + Number(this.transferDetails.contractDuration);
      ...
      axios({
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            this.messages.push({
              user: "manager",
              text: `好的，基本情况已经确定，${this.playerName}对于加盟贵队非常感兴趣，您给出的条件看起来非常合理。我们已经与${this.playerName}和他的经纪人讨论了这份提议，他们没有异议，我们将接受这个方案`,
            });
    ...
    querySearch(queryString, cb) {
      ...
      var results = queryString
        ? inputprompt.filter(this.createFilter(queryString))
        : inputprompt;
    ...
    createFilter(queryString) {
      return (prompt) => {
        return (
          prompt.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0
        );
      };
    },
    additionalFilter(prompt) {
      ...
      if (
        key &&
        this.transferDetails[key] !== null &&
        this.transferDetails[key] !== ""
      ) {
        return false;
      }
    ...
    handleSelect(item) {
      ...
      if (value === "转会费") {
        this.input = `我们愿意提供${
          this.transferDetails.fee === null ? "[XXX]" : this.transferDetails.fee
        }万元作为基础转会费，并可以考虑加入一些附加条款来满足您的要求。`;
      } else if (value === "工资") {
        this.input = `我们愿意提供${
          this.transferDetails.salary === null
            ? "[XXX]"
            : this.transferDetails.salary
        }万元作为工资，我们相信这样的条件能够体现他对球队的价值，并且符合我们俱乐部的财务规划。`;
      } else if (value === "转会窗口") {
        this.input = `我们倾向于在今年${
          this.transferDetails.transferWindow === ""
            ? "[XXX]"
        ...
        this.input = `我们计划提供一份合同时长为${
          this.transferDetails.contractDuration === null
            ? "[XXX]"
            : this.transferDetails.contractDuration
        }年的合同。`;
      }
    },
    loadAllprompts() {
